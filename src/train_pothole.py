...
def download_dataset(api_key, dest_dir, retries=5):
    ...
    # the identical download_dataset() in src/train_signs.py: the
    # same version.download('yolov8') call has succeeded outright in
    # one run and produced an export with no data.yaml anywhere in it
    # in another, back to back, same key, same code. So the data.yaml
    # check below has to be *inside* the retry loop, not just the
    # exception handling -- a "successful" call that produced nothing
    # useful is exactly the failure mode this needs to retry past, not
